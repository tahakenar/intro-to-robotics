...
enum Frame { INITIAL = 0, CURRENT_FRAME = 1, FIXED_FRAME = 2 };
}  // namespace transform_enums
...
class Transform {
 ...
 public:
  ...
  /**
   * @brief Adds homogeneous transform to the query. Takes the frame type
   * (FIXED/CURRENT) into account
   *
   ...
   * @param trig_results Pair of cos(theta) (first) and sin(theta) (second) to
   * be used to represent rotation
   */
  void getXRotation(Eigen::Matrix4d& m,
   ...
   * be used to represent rotation
   */
  void getYRotation(Eigen::Matrix4d& m,
                    const std::pair<double, double>& trig_results);
   ...
   * be used to represent rotation
   */
  void getZRotation(Eigen::Matrix4d& m,
                    const std::pair<double, double>& trig_results);
